...
class _ProfileTabState extends State<ProfileTab> {
  ...
  Future<void> _loadNotificationPreference() async {
    final isEnabled = await _notificationService.getNotificationsEnabled();
    if (!mounted) {
      return;
    }
    setState(() {
  ...
  Future<void> _setNotificationsEnabled(bool value) async {
    final result = await _notificationService.setNotificationsEnabled(value);
    if (!mounted) {
      return;
    }
    setState(() {
      _notificationsEnabled = result.isEnabled;
    });
    if (result.message != null && result.message!.isNotEmpty) {
      context.showSuccessMessage(result.message!);
    }
  }

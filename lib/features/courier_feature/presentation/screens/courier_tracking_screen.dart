...
class _CourierTrackingScreenState extends State<CourierTrackingScreen> {
  ...
  bool _isTrackable(CourierOrderModel o) {
    return o.status == CourierOrderStatus.assigned ||
        o.status == CourierOrderStatus.pickedUp ||
        o.status == CourierOrderStatus.inTransit;
  }

  /// Tam ekran (selectedOrder) ile Harita sekmesi aynı listeyi kullanır; seçili sipariş öne alınır.
  List<CourierOrderModel> _trackableOrdersForMap(
    List<CourierOrderModel> orders,
  ) {
    final list = orders.where(_isTrackable).toList();
    final sel = widget.selectedOrder;
    if (sel != null && _isTrackable(sel)) {
      list.sort((a, b) {
        final aSel = _orderIdEq(a.id, sel.id);
        final bSel = _orderIdEq(b.id, sel.id);
  ...
  Future<void> _syncTrackingWithOrdersList(
    ...
    final active = _trackableOrdersForMap(orders);

    if (active.isEmpty) {
      if (loc.state.status == CourierLocationStatus.tracking) {
        await loc.stopTracking(userInitiated: false);
      }
      if (mounted) {
        setState(() {
          _clearRouteState();
    ...
    final wantId = active.first.id;

    if (loc.suppressAutoRestart &&
        loc.state.status != CourierLocationStatus.tracking) {
      return;
    }

    final curId = loc.activeTrackingOrderId;
    ...
    await loc.startTracking(orderId: wantId);
  }
    ...
    try {
      ...
      if (result != null) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          final orders = context.read<CourierOrdersCubit>().state;
          final fo = _focusedOrder(_trackableOrdersForMap(orders));
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, _) {
        if (didPop && widget.selectedOrder != null) {
          unawaited(_locationCubit.stopTracking(userInitiated: true));
        }
      },
      child: BlocListener<CourierOrdersCubit, List<CourierOrderModel>>(
        ...
        child: BlocConsumer<CourierLocationCubit, CourierLocationState>(
          listener: (context, locState) {
            ...
            final orders = context.read<CourierOrdersCubit>().state;
            final mapOrder = _focusedOrder(_trackableOrdersForMap(orders));
            final dest = mapOrder != null ? _getCustomerLatLng(mapOrder) : null;
            if (locState.status == CourierLocationStatus.tracking) {
              var rotation = 0.0;
          ...
          builder: (context, locState) {
            return BlocBuilder<CourierOrdersCubit, List<CourierOrderModel>>(
              builder: (context, orders) {
                final activeOrders = _trackableOrdersForMap(orders);
                final mapOrder = _focusedOrder(activeOrders);
                ...
                return AppScaffold(
                  ...
                  body: Stack(
                    ...
                    children: [
                          ...
                          children: [
                            ...
                            MarkerLayer(
                              markers: [
                                ...
                                if (primaryCustomerLL != null)
                                  Marker(
                                    point: primaryCustomerLL,
                                    width: 44,
                                    height: 44,
                                    child: GestureDetector(
                                      behavior: HitTestBehavior.opaque,
                      ...
                      if (mapOrder != null && _showDeliverySummary)
                        Positioned(
                          left: 14,
                          right: 14,
                                ...
                                Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      if (mapOrder.customerPhone != null &&
                                      ...
                                      if (mapOrder.restaurantAddress != null &&
